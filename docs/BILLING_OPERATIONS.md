...
- 현재 결제수단은 무통장 입금이며, 입금 승인 전에는 교재 제작과 자료 다운로드가 제한됩니다.
- `BILLING_PG_CHECKOUT_ENABLED=false`가 기본값이라 Toss·KakaoPay 호출은 비활성 상태입니다.
...
- 무통장 입금 안내 계좌는 `api/_lib/billing/config.mjs`의 공개 설정 한 곳에서 관리하며, PG 정산 계좌와 Secret은 별개입니다.
...
| 프로모션 가격 | 18,000 KRW |
| 현재 결제액 | 18,000 KRW |
| 이용 기간 | 관리자 입금 승인일부터 달력 1개월 |
| 자동 갱신 | 없음, 매회 입금 신청 |

클라이언트가 보낸 `amount`는 받지도, 사용하지도 않습니다. 모든 승인 요청은 서버가 플랜 문서를 다시 조회해 금액을 결정합니다.
기존 PG 무료체험 필드는 Toss·KakaoPay 전환을 위해 보존하지만, 무통장 입금 운영에서는 무료 이용권을 발급하지 않습니다.

## 구조

### 사용자 흐름

1. `/billing`이 서버가 정한 금액과 입금 계좌, 현재 구독 상태를 조회합니다.
2. 사용자가 실제 입금자명을 입력하고 입금 신청을 제출합니다.
3. 서버는 `bank_transfer_requests/{uid}`에 1건의 대기 신청만 저장합니다.
4. 마스터가 `/admin/billing`에서 실제 입금을 확인하고 승인합니다.
5. 승인 트랜잭션이 결제 내역과 1개월 `active` 구독을 함께 저장합니다.
6. 승인 전에는 교재 생성 API, 공식 자료 서명 URL, Firebase Storage 다운로드가 모두 거부됩니다.

Toss·KakaoPay checkout 코드는 향후 전환을 위해 보존되며 `BILLING_PG_CHECKOUT_ENABLED=true` 전에는 호출할 수 없습니다.

### 만료 처리

- Vercel Cron이 매일 `01:00 UTC`(`10:00 KST`)에 `/api/billing-cron`을 GET으로 호출합니다.
- `Authorization: Bearer ${CRON_SECRET}`이 정확히 일치해야 실행됩니다.
- `nextBillingAt <= now`인 구독을 최대 100건씩 처리합니다.
- 무통장 입금 구독은 PG를 호출하지 않고 `past_due`로 바꾸며 즉시 이용 권한을 회수합니다.
- Toss·KakaoPay가 활성화된 경우에만 기존 자동 청구와 재시도 로직을 사용합니다.
...
| `bank_transfer_requests` | 입금자명, 고정 금액, 승인·반려 상태 | 읽기·쓰기 거부 |

서버 전용 collection은 Firebase Admin SDK만 접근합니다. 관리자 화면도 Client SDK로 이 문서들을 직접 읽지 않고 `/api/billing`의 관리자 전용 액션을 사용합니다.

## 상태 전이

- 무통장 입금 신청: `pending` -> 관리자 승인 후 `active`
- 한 달 만료: `active` -> `past_due`, 즉시 이용 제한
- 재입금 승인: 새 1개월 활성화, 이용 중 선입금이면 남은 기간 뒤로 한 달 연장
...
BANK_TRANSFER_BANK_NAME=지역농협
BANK_TRANSFER_ACCOUNT_NUMBER=3521660492353
BANK_TRANSFER_ACCOUNT_HOLDER=
BILLING_PG_CHECKOUT_ENABLED=false
...
2. 무통장 입금 신청과 관리자 승인, 만료, 재입금 연장을 테스트합니다.
3. 교재 생성 API와 Storage 다운로드가 승인 전 거부되는지 확인합니다.
4. `BILLING_PG_CHECKOUT_ENABLED=false`를 확인합니다.
...
8. PG 전환이 끝날 때 무통장 입금 신규 신청을 닫고 기존 이용기간을 보존합니다.
...
자동 테스트는 기존 PG 안전장치와 무통장 입금 승인, 한 달 만료, 재입금 연장을 포함해 총 29개를 실행합니다.
