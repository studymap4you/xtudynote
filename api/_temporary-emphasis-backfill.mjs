...
function conservativeMarkerRanges(problem, passage) {
  ...
  for (let i = 0; i < markers.length; i += 1) {
    ...
    const segment = passage.slice(start0, endLimit).trimStart();
    const leadingWhitespace = passage.slice(start0, endLimit).length - passage.slice(start0, endLimit).trimStart().length;
    const start = start0 + leadingWhitespace;
    if (!segment) continue;

    // Last-resort legacy recovery only. Prefer a short grammatical/lexical unit rather than bolding the whole interval.
    ...
    if (candidateEnd <= 0 && wordTokens.length) candidateEnd = Number(wordTokens[Math.min(2, wordTokens.length - 1)].index) + wordTokens[Math.min(2, wordTokens.length - 1)][0].length;
...
export default async function handler(req, res) {
  ...
  try {
    ...
    console.error("[temporary-emphasis-backfill]", error);
