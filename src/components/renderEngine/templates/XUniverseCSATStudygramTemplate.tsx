...
import type { CSATRenderPage, CSATRenderUnit, PreparedCSATBooklet } from "@/lib/renderEngine/types";
...
function CoverPage({ booklet, scale }: { booklet: PreparedCSATBooklet; scale: number }) {
  return (
    <div
    ...
    >
      <article className={`${styles.page} ${styles.coverPage} csat-page`} style={{ transform: `scale(${scale})` }}>
        <div className={styles.coverTopline}><b>XUniverse Learning</b><span>Studygram Edition · v1</span></div>
        <div className={styles.coverSquares} aria-hidden="true">
          {Array.from({ length: 8 }, (_, index) => <i key={index} />)}
        </div>
        <div className={styles.coverHero}>
          <small>HIGH SCHOOL ENGLISH</small>
        ...
        <div className={styles.coverDetails}>
          ...
          <section><b>FOCUS</b><strong>{booklet.subtitle || "Reading · Reasoning · Accuracy"}</strong><span>Validated question set</span></section>
          <section><b>EDITION</b><strong>{booklet.options.mode === "student" ? "Student Book" : "Review Book"}</strong><span>XUniverse original layout</span></section>
          <section className={styles.coverMicrocopy}><b>ONE MORE</b><strong>한 문제씩, 점수는 쌓인다.</strong><span>Keep going.</span></section>
        </div>
        <div className={styles.coverFooterBand}><span>XUNIVERSE LEARNING</span><b>ENGLISH CSAT · PRACTICE SET</b></div>
...
export type XUniverseCSATStudygramTemplateProps = {
  booklet: PreparedCSATBooklet;
  pages: CSATRenderPage[];
  scale: number;
};

export function XUniverseCSATStudygramTemplate({ booklet, pages, scale }: XUniverseCSATStudygramTemplateProps) {
  const showAnswerKey = booklet.options.mode === "review" && booklet.options.showAnswerKey;
  return (
    <div className={`${styles.pageStack} csat-page-stack`}>
      <CoverPage booklet={booklet} scale={scale} />
        ...
        <CSATPage key={page.id} pageNumber={index + 2} section={`Practice Set · ${booklet.questions.length} Questions`} scale={scale}>
          {page.units.map((unit, unitIndex) => (
            <RenderUnit
              ...
              unit={unit}
              booklet={booklet}
              previousUnit={page.units[unitIndex - 1]}
              nextUnit={page.units[unitIndex + 1]}
            />
          ))}
        </CSATPage>
      ))}
...
export { RenderUnit as XUniverseCSATRenderUnit };
